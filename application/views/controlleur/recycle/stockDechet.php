<div class="card w-100 m-0 p-0">
	<div class="card-header bg-dark text-white">
		...
		<b class="w-100 border-dark text-right col-md-6 pull-right">
			...
			<a href="#" class=" text-white ml-2 btn btn-success btn-sm afficher"><i class="fa fa-tv"></i>&nbsp; AFFICHER</a>
			<a href="<?= base_url("Recyclage/entreDechetExport") ?>" class="exportDechet text-white ml-2 btn btn-warning btn-sm"><i
					class="fas fa-file-excel"></i>&nbsp; EXPORTE</a>
			<a href="<?= base_url("Recyclage/entreDechetPrint") ?>" class="printDechet text-white ml-2 btn btn-info btn-sm"><i
...
<script>
	$(() => {
		...
		Table = $('.tableDechet').DataTable({
			processing: true,
			ajax: base_url + "Recyclage/entreDechet?debut=&fin=",
			language: {
...
$('.afficher').on('click',function(e){
      e.preventDefault()
			let debut = $('.debut').val()
			let fin = $('.fin').val()
			$('.exportDechet').attr('href',base_url+"Recyclage/entreDechetExport?debut="+debut+"&fin="+fin)
			$('.printDechet').attr('href',base_url+"Recyclage/entreDechetPrint?debutPrint="+debut+"&fin="+fin)
