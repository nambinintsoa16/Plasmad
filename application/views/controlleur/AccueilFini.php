...
<div class="card-body">
    <select class="form-control choix">
        <option>STOCKS PRODUITS FINIS</option>
        <option>STOCKS PRODUITS EN SURPLUS</option>
    </select>
<fieldset class="col-md-12 border mt-2 w-100 ">
...
<a href="" class="btn btn-primary imprimerprecosting"><i class="fa fa-print"></i>&nbsp;Imprimer
						precosting</a>
<a href="" class="btn btn-info imprimer w-25"><i class="fa fa-print"></i>&nbsp;Imprimer</a>
</fieldset>	
	</div>
	</div>  
 </div>  
...
$(document).ready(function(){
 ...
 Tablesx = $(".tableProduitFini").dataTable({
    processing: true,
    ajax:base_url+"Magasiner/dataProduitFinis",
    "rowCallback": function (row, data) {
        $('.infoProduit').on('click',function(event){
      event.preventDefault();
      var pe = $(this).attr('href');
      $.post(base_url+"Stock/detailBon",{pe:pe},function(data){
 ...
 $('.choix').on('change',function(){
     ...
     if(choix=="STOCKS PRODUITS FINIS"){
        var  links = base_url +"Magasiner/dataProduitFinis";     
     }else{
