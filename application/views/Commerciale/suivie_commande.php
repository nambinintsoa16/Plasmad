...
<div class="card">
	<div class="card-header bg-dark text-white">
			<b>SUIVIE COMMANDE</b>
		<p class="float-right m-0 p-0">Début : <input type="date" class="debut"> Fin : <input type="date" class="fin">N°PO : <input type="text" class="po"> Code client : <input type="text" class="client"> <button type="submit" class="btn btn-sm btn-info afficher">Affiche</button> <a href="#" class="btn btn-success btn-sm Exporter">Exporter</a> </p>
	</div>
	<div class="card-body">
		<div class="form">
		<div class="row">
...
<script>
	$(document).ready(function() {
       ...
       $('.Exporter').attr('href', base_url + 'Commerciale/suivieExport');
		let link = base_url + 'Commerciale/suivie';
		...
		$('.po').autocomplete({
         ...
         $('.saveModif').on('click',function(e){
			...
			$.post(base_url+'Suivi_Commerciale/saveSuivie',{idModifMod:idModifMod,itemModif:itemModif,valeurModif:valeurModif},function(data){
				...
				$('.valeurModif').val("");
				$('.Exporter').attr('href', base_url + 'Commerciale/suivieExport?debut=' + debut + '&fin=' + fin + '&po=' + po+'&client='+client);
				let links = base_url + 'Commerciale/suivie?debut=' + debut + '&fin=' + fin + '&po=' + po+'&client='+client;
