	...
	$(document).ready(function() {
		function deleteTd() {
			$('.dell').on('click', function(event) {
				...
				$.post(base_url + 'planning/deleTeMatier', {
					id: id
				}, function() {
					parent.remove();
					swal("succè! ", "Matière supprimée", {
						icon: "success",
						buttons: {
							confirm: {
		...
		$('.DJ_MACHINEE').on('change', function() {
			...
			$.post(base_url + 'Planning/calculePlanning', {
				...
				if (data.message == "init") {
					var debut = $('.hdeb').val();
					var dure = $('.heure').val();
					$.post(base_url + "Planning/crateJobsMachine", {
						debut: debut,
						dure: dure,
						BC: BC,
						machine: machine,
		...
		$('.BC_POISENKGSAVECMARGE').on('change', function() {
			...
			var data = $(this).val();
			var BC = $('.BC_PE').val();
			$.post(base_url + 'Planning/updatPoid', {
				data: data,
				BC: BC
			}, function(data) {
				if (data == 1) {
					swal("succè! ", "POIDS EN KG AVEC MARGE modifié", {
						icon: "success",
						buttons: {
							confirm: {
								className: 'btn btn-success'
					...
					swal("Erreur! ", "POIDS EN KG AVEC MARGE non modifié", {
						icon: "error",
						buttons: {
							confirm: {
		...
		$('.BC_QUANTITEAPRODUIREENMETRE').on('change', function() {
			...
			var data = $(this).val();
			var BC = $('.BC_PE').val();
			$.post(base_url + 'Planning/updatemetre', {
				data: data,
				BC: BC
			}, function(data) {
				if (data == 1) {
					swal("succè! ", "Quantité modifiée", {
						icon: "success",
						buttons: {
							confirm: {
								className: 'btn btn-success'
					...
					swal("Erreur! ", "Quantité non modifiée", {
						icon: "error",
						buttons: {
							confirm: {
								className: 'btn btn-danger'
		...
		$('#TERMINER').on('change', function(event) {
			...
			var data = $(this).val();
			var BC = $('.JO_ID').val();
			$.post(base_url + 'Planning/updateteminer', {
				data: data,
				BC: BC
			}, function(data) {
				if (data == 1) {
					swal("succè! ", "Poids modifié", {
						icon: "success",
						buttons: {
							confirm: {
								className: 'btn btn-success'
		...
		$('.add-table').on('click', function(e) {
			...
			if ($('.stock_sortie').val() != "" && $('.Quantite').val() != "") {
				...
				if (typeof($('.tbody-modaltable > tr').html()) == 'undefined') {
					...
					var BC = $('.BC_PE').val();
					$.post(base_url + "planning/sauveMatier", {
						DESIGNATION: DESIGNATION,
						QUANTITE: QUANTITE,
						BC: BC
					}, function(Response) {
						swal("succè! ", "matière ajoutée", {
							icon: "success",
							buttons: {
								confirm: {
									className: 'btn btn-success'
					...
					if ($.inArray(data[0], table) != -1) {
						alertMessage("Ooops!!", "Ce matière existe déjà dans votre bon de commande. Veuillez modifier la quantité pour ajouter une autre matière.", 'error', 'btn btn-danger');
					} else {
						i++;
						$('.tbody-modaltable').append('<tr><td>' + i +
							'</td><td>' + data[0] + '</td>' +
							'<td>' + $('.Quantité').val() + '</td><td class="text-center"><a href="#" class="btn btn-danger btn-sm dell"><i class="fa fa-times"></i></a>' +
							'</td></tr>');
				...
				alertMessage("Ooops!!", "Champs désignation et Champs Quantité obligatoire", 'error', 'btn btn-danger');
			}
		});
		function chargement() {
