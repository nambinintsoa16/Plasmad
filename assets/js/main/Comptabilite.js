$(document).ready(function() {
    chargement();
    $.post(base_url + "Comptabilite/page", { page: "MATIERE PREMIERE" }, function(data) {
        $('.main').empty().append(data);
        $('.link').text("ACCUEIL/MATIERE PREMIERE" );
        matierPremier();
        closeDialog();
    });
    ...
    $('.btn-click').on('click', function(e) {
        ...
        chargement();
        $.post(base_url + "Comptabilite/page", { page: page }, function(data) {
            $('.main').empty().append(data);
            $('.link').text(page);
    ...
    function sortie() {
        ...
        $('.saveTransaction').on('click', function(e) {
            ...
            JSON.s
            $.post(base_url + 'Comptabilite/saurSortie', { data: donne, date: datesortie, reception: reception, PE: PE }, function(data) {
    ...
    function actionUpdate(id) {
        $('.validerModif').on('click', function(e) {
            e.preventDefault();
            var designation = $('.designation').val();
            var prixUSD = $('.prixUSD').val();
            var prixAriary = $('.prixAriary').val();

            $.post(base_url + 'Comptabilite/UpdateRowMatier', {
                designation: designation,
                prixUSD: prixUSD,
                id: id,
                prixAriary: prixAriary
            }, function(data) {
                ...
                if (data == 1) {
                    ...
                    $('.prixUSD').val();
                    $('.prixAriary').val();
                    $('.modifimatier').modal('hide');

                    swal("Succé ", "Modification effectuer avec succé!", {
                        icon: "success",
                        buttons: {
                            confirm: {
    ...
    function actionMatierPremier() {
        var table = $('.dataTable').dataTable({
            processing: true,
            ajax: "Comptabilite/liseteMPremierData",
            language: {
            ...
            initComplete: function(setting) {
                $('.editMatiert').on('click', function(e) {
                    e.preventDefault();
                    $this = $(this).parent().parent().children();
                    $('.designation').val($this.eq(0).text());
                    $('.prixUSD').val($this.eq(2).text());
                    $('.prixAriary').val($this.eq(3).text());
                    $('.modifimatier').modal('show');
                    actionUpdate();
                });
        ...
        $('form').on('submit', function(event) {
            ...
            $.ajax({
                ...
                url: base_url + 'Comptabilite/update_matierPremier',
                type: 'post',
                data: fd,
                contentType: false,
                processData: false,
        ...
        $('.saveMatier').on('click', function(e) {
            e.preventDefault();
            var designation = $('.designationMat').val();
            var prixUSDX = $('.prixUSDX').val();
            var prixAriaryX = $('.prixAriaryX').val();
            $.post(base_url + 'Comptabilite/saveMatierNew', { designation: designation, prixUSDX: prixUSDX, prixAriaryX: prixAriaryX }, function(data) {
                $('input').val();
    ...
    function sortieMPremier() {
        $.post(base_url + "Comptabilite/page", { page: "SORTIE_ECHANGE" }, function(data) {
        ...
        $('.typeSortie').on('change', function(event) {
            ...
            $.post(base_url + "Comptabilite/page", { page: "SORTIE_" + page }, function(data) {
                $('.contTypeAction').empty().append(data);
    ...
    function echange() {
        ...
        $('.saveEchange').on('click', function(event) {
            ...
            $.post(base_url + "Comptabilite/echange", { fornisseur: fornisseur, produitForn: produitForn, quantiteForn: quantiteForn, receptQuantite: receptQuantite, receptProduit: receptProduit, recept: recept }, function(data) {
                if (data.type == "error") {
    ...
    function valideSortie() {
        $('.sortierMatier').on('click', function(event) {
            ...
            swal({
                ...
                if (Delete) {
                    chargement();
                    $.post(link, { po: po, matier: parent, quantite: quantite }, function(data) {
                        $.post(base_url + 'Comptabilite/chercheMateir', { po: po }, function(datas) {
    ...
    function matierPremier() {
        chargement();
        $.post(base_url + "Comptabilite/page", { page: "STOCK MATIERE PREMIERE" }, function(data) {
    ...
    function produit() {
        $.post(base_url + "Comptabilite/page", { page: "STOCK PRODUIT FINI" }, function(data) {
            $('.main-matier').empty().append(data);
            produitFini();
        });
    ...
    function SORTIESTK() {
        $('.auto').autocomplete({
            source: base_url + "Comptabilite/autocompletSortie",
            select: function(data, iteme) {
                var param = iteme.item.value.trim();
                $.post(base_url + "Comptabilite/recherchePESort", { param: param }, function(data) {
        ...
        $('form').on('submit', function(event) {
            ...
            $.ajax({
                type: 'POST',
                url: base_url + "Comptabilite/savesortie",
                data: formData,
                cache: false,
                contentType: false,
                processData: false,
    ...
    function actinApprove() {
        $('.matier').autocomplete({
            source: base_url + "Comptabilite/autocompleteSortie",
        });
        $('form').on('submit', function(event) {
            event.preventDefault();
            var designation = $('.designation').val();
            ...
            var reference = $('.reference').val();
            $.post(base_url + "Comptabilite/saveApprove", { reference: reference, designation: designation, quantite: quantite, date: date, forniseur: forniseur }, function(data) {
                if (data.type == "error") {
    ...
    function cherchePo() {
        ...
        $('.recherchePosBtn').on('click', function(event) {
            event.preventDefault();
            var po = $('.recherchePos').val();
            $.post(base_url + 'Comptabilite/chercheMateir', { po: po }, function(data) {
                $('.tbody-matirUtiliser').empty().append(data);
                valideSortie();
            });
        });
    }

    function valideSortieBon() {
