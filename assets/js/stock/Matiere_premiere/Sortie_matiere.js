$(document).ready(function() {
    ...
    $('#matiere_designation').autocomplete({
        source: base_url + "stock/autocomplete_matiere_sortie_magasin_quantite",
        select: function(event, items) {
            event.preventDefault();
            let select_item = items.item.value.split(" | ");
            $('#refmum_matiere').val(select_item[0]);
            if(select_item[2].trim() > 0){
              $(this).val(select_item[1] + " | " + select_item[3]);
              $('#quantite_disponible').empty().append(select_item[2].trim());
            }
            if(select_item[2].trim() <= 0){
                $(this).val(" ");
                alertMessage("Erreur!","Stock insuffisant", "error", "btn btn-danger");
    ...
    $('#add-table').on('click', function(e) {
        ...
        let methodOk =  false;
        let quantite_disponible =  $('#quantite_disponible').text(); 
        let quantite_entre = $('#quantite_matiere').val()

        methodOk = parseFloat(quantite_disponible) > parseFloat(quantite_entre); 
       if(methodOk==false){
          alertMessage("Erreur!","Stock insuffisant", "error", "btn btn-danger");
        }
            ...
            if ($('#matiere_designation').val() != "" && $('#quantite_matiere').val() != "") {
                ...
                if (typeof($('#table_content > tr').html()) == 'undefined') {
                    $('#table_content').append('<tr><td>' + refnum + '</td><td>' + machine + '</td><td>' + refnum_matiere + '</td><td>' + data[0].trim() + '</td>' +
                        '<td><input type="number" class="qttable w-10 text-center" disabled style="width:80px;" value="' + $('#quantite_matiere').val() +
                        '"></td><td>' + i +
                    ...
                    deleteTd();
                    $('#quantite_disponible').empty();

                } else {
                    let table = [];
                    let data = $('#matiere_designation').val().split("|");
                    $('#table_content > tr').each(function() {
                        table.push($(this).children().eq(3).text().trim());
                    });
                    if ($.inArray(data[0].trim(), table) != -1) {
