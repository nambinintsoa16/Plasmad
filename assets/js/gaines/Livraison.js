$(document).ready(function() {
    ...
    $("#refnum_commande").autocomplete({
        source: base_url + "Gaines/autocomplete_produit_gaine",
        select: function(data, iteme) {
            let refnum_pe = iteme.item.value.trim();
            $.post(
                base_url + "Gaines/detail_commande_livraison_gaine", { refnum_pe },
                function(data) {
                    if (data.mesage == "false") {
                        ...
                        data.tail.forEach((element) =>
                            $(".tail").append("<option>" + element.tail + "</option>")
                        );
                        table.ajax.url(
                            base_url +
                            "stock/data_liste_sortie_surplus_livraison?refnum=" +
                            refnum_pe
                        );
                        table.ajax.reload();
                    }
    ...
    $(".tail").on("change", function(event) {
        ...
        let tail = $(this).val();
        $.post(
            base_url + "gaines/get_quantite_dispo_stock", { refnum, tail },
            function(data) {
    ...
    $(".surplus").autocomplete({
        ...
        select: function(data, iteme) {
            ...
            $.post(
                base_url + "stock/taille_produit_surplus", { refnum },
                function(data) {
                    if (data.mesage == "false") {
                        alertMessage(
                            "Erreur!",
                        ...
                        $("#tail_suplus").empty();
                        data.tail.forEach((element) =>
                            $("#tail_suplus").append("<option>" + element.tail + "</option>")
                        );
                    }
                },
                "json"
    ...
    $("#save_sortie").on("click", function(event) {
        ...
        if (date_entre == "") {
            ...
            if (quantite_sortie != "") {
                $.post(
                    base_url + "gaines/sortie_ganes", { refnum, tail, quantite_sortie, date_entre, BL, obs },
                    function() {
                        if (quantite_surplus != "") {
                            $.post(
                                base_url + "stock/sortie_suplus_fini", {
                                    tail_suplus,
                                    refnum,
                                    quantite_surplus,
                                    refnum_surplus,
