$(document).ready(function() {
    var table = $("#dataTable").DataTable({
        processing: true,
        ajax: base_url + 'Controlleur/data_list_Prix',
        language: {
    ...
    function delete_function() {
        $(".edit_statut").on('click', function(event) {
            event.preventDefault();
            let refnum = $(this).attr('id');
            $.post(base_url + "controlleur/disable_prix_appliquer", { refnum }, function() {

            }).done(() => {
                table.ajax.reload();
                alertMessage("Succè!", "Prix désactivé.", "success", "btn btn-success");
            }).fail(() => {
